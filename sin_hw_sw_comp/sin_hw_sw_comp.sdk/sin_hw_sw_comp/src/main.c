...
int main (void) 
{
	...
	uint32_t * vref_hw_sin_lut = 0;

	for (int i = 0; i <80; i++)
	{
		vref_sw_sin_math = sin_math(current_time);
		vref_sw_sin_lut = sin_from_lut(current_time);
		vref_hw_sin_lut = sin_from_hw_lut(current_time);

		//printf("%i: vref_sw_sin_math: %f, %f, %f, %f, %f\r\n", i, vref_sw_sin_math[0], vref_sw_sin_math[1], vref_sw_sin_math[2], vref_sw_sin_math[3], vref_sw_sin_math[4]);
		...
		//printf("vref_hw_sin_lut: %f, %f, %f, %f, %f\r\n\r\n", (float)vref_hw_sin_lut[0]/pow(2,30), (float)vref_hw_sin_lut[1]/pow(2,30), (float)vref_hw_sin_lut[2]/pow(2,30), (float)vref_hw_sin_lut[3]/pow(2,30), (float)vref_hw_sin_lut[4]/pow(2,30));
